...
describe("supports ansi-styles", () => {
  test("supports style.red", async () => {
    ...
    ).to.equal("<red> foo content </color>");
  });
  ...
  test("supports style.green", () => {
    ...
    ).to.equal("<green> foo content </color>");
  });
  ...
  test("supports style.reset", () => {
    ...
    ).to.equal("</> foo content </>");
  });

  test("supports style.bold", () => {
    expect(prettyAnsi(`${ansiStyle.bold.open} foo content`)).to.equal(
      "<bold> foo content",
    );
  });

  test("supports style.dim", () => {
    expect(prettyAnsi(`${ansiStyle.dim.open} foo content`)).to.equal(
      "<dim> foo content",
    );
  });
